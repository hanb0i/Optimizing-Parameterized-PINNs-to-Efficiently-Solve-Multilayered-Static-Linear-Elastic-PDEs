"""Evaluate PINN-vs-FEM generalization on random interior parameter cases."""
...
def main() -> None:
    ...
    parser.add_argument("--out-csv", default=str(GRAPHS_DATA_DIR / "random_interior_generalization.csv"))
    parser.add_argument("--out-summary", default=str(GRAPHS_DATA_DIR / "random_interior_generalization_summary.json"))
    ...
    for case in random_interior_cases(args.n_cases, args.seed):
        ...
        print(
            f"{case.case_id}: top MAE={result['top_uz_mae_pct']:.2f}% "
            f"volume MAE={result['volume_mae_pct']:.2f}%"
        )
