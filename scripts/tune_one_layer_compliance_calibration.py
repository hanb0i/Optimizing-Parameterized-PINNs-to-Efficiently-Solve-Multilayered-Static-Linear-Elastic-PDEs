"""Tune one-layer PINN compliance scaling against FEM references."""
...
def _prepare_case(pinn, device, case: OneLayerCase, ne_x: int, ne_y: int, ne_z: int) -> dict:
...
def main() -> None:
    ...
    parser.add_argument("--out", default=str(GRAPHS_DATA_DIR / "one_layer_compliance_calibration.json"))
    ...
    calibration_cases = [
        OneLayerCase("one_layer_grid_soft", 1.0, 0.15),
        OneLayerCase("one_layer_grid_stiff", 10.0, 0.05),
    ...
    initial = np.array(
        [
            ...
            float(getattr(config, "E_COMPLIANCE_POWER", 1.0)),
            float(getattr(config, "THICKNESS_COMPLIANCE_ALPHA", 0.0)),
        ],
        dtype=float,
    )
    bounds = [(0.1, 4.0), (0.0, 1.6), (0.0, 4.5)]
    de = opt.differential_evolution(objective, bounds=bounds, seed=args.seed, maxiter=args.maxiter, polish=False)
    local = opt.minimize(objective, de.x, method="Nelder-Mead", options={"maxiter": 250, "xatol": 1e-4, "fatol": 1e-4})
    ...
    payload = {
        ...
        "note": "This tunes only the existing PINN compliance map. The PINN remains the solver; FEM is the reference.",
    }
