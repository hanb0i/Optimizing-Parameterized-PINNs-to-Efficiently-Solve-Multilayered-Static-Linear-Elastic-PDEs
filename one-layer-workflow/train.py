...
def _artifact_dir():
    out_dir = os.getenv("PINN_OUT_DIR", BASE_DIR)
...
def train():
    ...
    epochs_adam = int(os.getenv("PINN_EPOCHS_ADAM", os.getenv("PINN_EPOCHS_SOAP", str(config.EPOCHS_ADAM))))
    ...
    if os.getenv("PINN_WARM_START", "1") == "1":
        ckpt_candidates = [
            os.getenv("PINN_MODEL_PATH", ""),
            _artifact_path("pinn_model.pth"),
            os.path.join(BASE_DIR, "pinn_model.pth"),
    ...
    try:
        fem_data = np.load(os.path.join(BASE_DIR, "fea_solution.npy"), allow_pickle=True).item()
        X_fea = fem_data['x']
        Y_fea = fem_data['y']
        Z_fea = fem_data['z']
        U_fea = fem_data['u']
    ...
    soap_time = time.time() - start_time
    print(f"SOAP Pretraining Complete. Total Time: {soap_time:.2f}s")
    ...
    timing = {
        ...
        "soap_training_seconds": float(soap_time),
        "lbfgs_training_seconds": float(lbfgs_time),
        "total_training_seconds": float(soap_time + lbfgs_time),
        "supervision_points": int(len(training_data.get("x_data", []))) if "x_data" in training_data else 0,
    }
    import json
    with open(_artifact_path("training_timing.json"), "w") as f:
