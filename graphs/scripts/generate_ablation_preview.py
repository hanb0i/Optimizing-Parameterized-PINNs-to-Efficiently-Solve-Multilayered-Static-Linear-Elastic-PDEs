#!/usr/bin/env python3
"""Generate a preview cross-section error heatmap for one-layer and three-layer."""
...
def _three_layer_preview():
    """Generate three-layer cross-section error heatmap."""
    ...
    # Representative case: thin-stiff outer, thick-soft middle
    e1, e2, e3 = 10.0, 1.0, 10.0
    t1, t2, t3 = 0.02, 0.10, 0.02
    thickness = t1 + t2 + t3
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("z")
    ax.set_title(f"Three-Layer Cross-Section Error (y=0.5)\nE=[{e1:g},{e2:g},{e3:g}], t=[{t1:g},{t2:g},{t3:g}]\nMean error: {float(np.mean(abs_err_cross)):.6f}")
    fig.tight_layout()
    fig.savefig(PREVIEW_DIR / "three_layer_cross_section_error.png", dpi=160)
    plt.close(fig)
    print(f"Saved: {PREVIEW_DIR / 'three_layer_cross_section_error.png'}")


def _one_layer_preview():
    """Generate one-layer cross-section error heatmap."""
    one_layer_dir = REPO_ROOT / "one-layer-workflow"
    fea_dir = REPO_ROOT / "fea-workflow" / "solver"

    # Remove three-layer-workflow from path to avoid import conflicts
    ...
    import importlib
    # Force re-import of model module from one-layer-workflow
    if "model" in sys.modules:
        del sys.modules["model"]
    if "pinn_config" in sys.modules:
        del sys.modules["pinn_config"]
    if "fem_solver" in sys.modules:
        del sys.modules["fem_solver"]
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("z")
    ax.set_title(f"One-Layer Cross-Section Error (y=0.5)\nE={E_val:g}, t={t_val:g}\nMean error: {float(np.mean(abs_err_cross)):.6f}")
    fig.tight_layout()
    fig.savefig(PREVIEW_DIR / "one_layer_cross_section_error.png", dpi=160)
    plt.close(fig)
    print(f"Saved: {PREVIEW_DIR / 'one_layer_cross_section_error.png'}")


if __name__ == "__main__":
    _three_layer_preview()
    _one_layer_preview()
