...
from _common import REPO_ROOT, apply_ieee_style, print_inputs_used, save_figure, watermark_placeholder
...
def _try_import_surrogate_modules():
    sw_dir = REPO_ROOT / "pinn-workflow-2layer"
...
def main() -> None:
    ...
    outputs_dir = REPO_ROOT / "pinn-workflow-2layer" / "surrogate_workflow" / "outputs"
    ...
    if not dataset_path.exists() or not model_path.exists():
        ...
        out_paths = save_figure(fig, "fig_surrogate_scatter_two_layer_placeholder")
        plt.close(fig)
        print("Wrote placeholder 2-layer surrogate verification figure.")
        for p in out_paths:
            print(f"  - {p}")
        return
    ...
    out_paths = save_figure(fig, "fig_surrogate_scatter_two_layer")
    plt.close(fig)
