        ...
        for m in matrix:
            ...
            try:
                ...
                if m.device.type == "mps":
                    ...
                    eye = torch.eye(m64.shape[0], device=m64.device, dtype=m64.dtype)
                    _, Q = torch.linalg.eigh(m64 + 1e-30 * eye)
                    Q = Q.to(m.dtype)
            Q = torch.flip(Q, [1])
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        for ind, (m, o) in enumerate(zip(matrix, orth_matrix)):
            ...
            if power_iter.device.type == "mps":
                power_iter_cpu = power_iter.detach().to("cpu")
                Q_cpu, _ = torch.linalg.qr(power_iter_cpu)
                Q = Q_cpu.to(device=power_iter.device, dtype=power_iter.dtype)
            else:
