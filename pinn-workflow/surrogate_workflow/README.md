...
Trains a small MLP surrogate that maps **three-layer** design parameters to a scalar
response metric computed from the trained parametric PINN.
...
python pinn-workflow/surrogate_workflow/verify_phase1.py
```

Quick smoke run:

```bash
python pinn-workflow/surrogate_workflow/run_phase1.py --regenerate --n-samples 60 --max-epochs 400
python pinn-workflow/surrogate_workflow/verify_phase1.py --grid 9
```
